...
export class OpenSubtitlesProvider {
   ...
   */
  static async getSubtitles(type: ContentType, id: string): Promise<SubtitleItem[]> {
    try {
      ...
      console.warn(`[OpenSubtitlesProvider] Failed to fetch subtitles for ${id}:`, (error as any)?.message);
      return [];
    }
  }
}
